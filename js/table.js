...
function updateQty(rowIndex,value){if(db[rowIndex]){pushUndo({type:'cell',rowIndex:rowIndex,col:'实际数量',oldValue:db[rowIndex]['实际数量'],newValue:value});db[rowIndex]['实际数量']=value;updateStats();scheduleAutoSave();showToast('数量已更新');hideKeyboard();}}
...
function updateCell(rowIndex,col,value){if(db[rowIndex]){pushUndo({type:'cell',rowIndex:rowIndex,col:col,oldValue:db[rowIndex][col],newValue:value});db[rowIndex][col]=value;rebuildSearchIndex();scheduleAutoSave();}}
